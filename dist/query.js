 ...
 */
export function extractBaseLegislation(raw) {
    const bases = new Set();
    for (const segment of (raw || '').split(';')) {
        ...
        if (!matched && s.length > 3 && !ONLY_SUBREFERENCE.test(s)) {
            ...
            if (clean && !ONLY_SUBREFERENCE.test(clean))
                bases.add(clean);
        }
    }
    return Array.from(bases);
}
const TYPE_TOKEN = /\bRDC\b|\bPORTARIA\b|\bDECRETO\b|\bLEI\b|\bNR\b|\bNBR\b|\bINSTRUCAO NORMATIVA\b|\bIN\b|\bRESOLUCAO\b|\bNOTA TECNICA\b|\bPARECER\b|\bCBO\b|\bRE\b/;
/**
 ...
 */
export function canonicalLegislationKey(raw) {
    const up = (raw || '')
        .toUpperCase()
        .normalize('NFD')
        .replace(/[̀-ͯ]/g, '');
...
export function findLegislation(mention) {
 ...
 */
export function matchesScope(entry, scope = {}) {
    const entryUF = toUF(entry.uf);
    if (!entryUF)
        return true; // federal/nacional
    if (entryUF !== toUF(scope.uf))
        return false;
    if (!entry.municipio)
        return true;
...
export function isApplicable(entry, scope = {}) {
    ...
    return Boolean(scope.segment) && segments.includes(scope.segment);
}
/** Normas sugeridas para um estabelecimento, em ordem alfabética. */
export function applicableLegislation(scope) {
    return LEGISLATION_LIBRARY.filter((entry) => isApplicable(entry, scope)).sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));
}
 ...
 */
export function searchLegislation(filter = {}) {
    ...
    return LEGISLATION_LIBRARY.filter((entry) => {
        if (!filter.incluirRevogadas && entry.status === 'revogada')
            return false;
        if (filter.status && !filter.status.includes(entry.status))
            return false;
        if (querFederal && toUF(entry.uf))
            return false;
        if (ufAlvo && !matchesScope(entry, { uf: ufAlvo, municipio: filter.municipio }))
            return false;
        if (filter.segment && !(entry.segments || []).includes(filter.segment))
            return false;
        if (termos.length) {
 ...
 */
export function formatAbnt(mention, entry) {
